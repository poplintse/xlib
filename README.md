...
| 组件 | 状态 | 版本 |
| --- | --- | --- |
| Android | active | `0.11.0` / build `69` |
| iOS | active | `0.11.0` / build `45` |
| Backend | active | `0.10.0` |
| macOS | planned | — |
| API contract | active | `v1` |

当前源码对应 `releases/0.11.0.yaml`，清单状态为 draft；0.10.0 是已发布的 SQLite 迁移基线。旧清单是历史发布记录，不作为当前能力需求。
...
make check-alpha
make release-check RELEASE=0.11.0
```

所有构建使用已准备的组件版本，不自动递增或重写版本文件。`VERSION` 只校验与源码版本一致；不同则拒绝构建。版本变化必须经过明确的发布准备。`make prepare-release` 只校验，不分配版本。

`make check`、`make check-alpha` 与 `make release-check` 默认检查 0.11.0，可用 `RELEASE=<版本>` 指定其他清单。直接运行脚本可设置 `RELEASE_VERSION`；`release-check.sh` 的显式参数优先。旧清单须配合对应源码检验，在当前源码上失败不表示应改写历史记录。缺少清单或版本不一致均会失败，不跳过检查。
