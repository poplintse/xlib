...
final class LocalDatabase extends SQLiteOpenHelper {
    ...
    private static final String[] LEGACY_PREFERENCE_KEYS = {
            "books", "bookmarks", "auto_toc", "app_theme", "keep_screen_on",
            "auto_page_interval", "sensitivity", "font_family", "font_size", "line_spacing",
            "sync_configured_email", "sync_email", "sync_device_id", "sync_device_name",
            "sync_server_url", "sync_started", "sync_active_server_url", "sync_active_email",
            "sync_active_device_name", "sync_remote_email", "sync_remote_items"
    };
    private static final String LEGACY_HASH_PREFIX = "sync_book_hash_";
    ...
    static LocalDatabase open(Context context, SharedPreferences preferences) throws Exception {
        ...
        try {
            ...
            database.cleanupLegacyIfEligible(sql, preferences);
            return database;
    ...
    private static void createLegacyCleanupTable(SQLiteDatabase db) {
        db.execSQL("create table if not exists legacy_cleanup (migration_id text primary key "
                + "references legacy_migrations(migration_id), source_fingerprint text not null, "
    ...
    synchronized boolean addBookmark(long bookId, long offset) {
        ...
        try (Cursor cursor = getReadableDatabase().rawQuery(
                "select coalesce(max(cast(local_id as integer)),0) from bookmarks", null)) {
            if (cursor.moveToFirst()) now = Math.max(now, cursor.getLong(0) + 1L);
        }
        ContentValues values = new ContentValues();
    ...
    private void cleanupLegacyIfEligible(SQLiteDatabase db, SharedPreferences preferences) throws Exception {
        String ledgerFingerprint;
        try (Cursor cursor = db.rawQuery(
                "select source_fingerprint from legacy_migrations where migration_id=?",
                new String[]{LEGACY_MIGRATION})) {
            if (!cursor.moveToFirst()) return;
            ledgerFingerprint = cursor.getString(0);
        }

        String state = null;
        try (Cursor cursor = db.rawQuery(
                "select state from legacy_cleanup where migration_id=?",
                new String[]{LEGACY_MIGRATION})) {
            if (cursor.moveToFirst()) state = cursor.getString(0);
        }
        if ("completed".equals(state)) return;

        if (state == null) {
            String currentFingerprint = legacyFingerprint(preferences);
            if (!ledgerFingerprint.equals(currentFingerprint)) {
                throw new IllegalStateException("legacy storage changed before cleanup");
            }
            verifyDatabaseForCleanup(db);
            ContentValues row = new ContentValues();
            row.put("migration_id", LEGACY_MIGRATION);
            row.put("source_fingerprint", ledgerFingerprint);
            row.put("state", "in_progress");
            row.put("started_at_ms", System.currentTimeMillis());
            row.put("removed_items", 0);
            db.insertOrThrow("legacy_cleanup", null, row);
        }

        int removed = removeLegacyPreferences(preferences) + removeLegacyTocFiles();
        verifyLegacyRemoved(preferences);
        verifyDatabaseForCleanup(db);
        ContentValues completed = new ContentValues();
        completed.put("state", "completed");
        completed.put("completed_at_ms", System.currentTimeMillis());
        completed.put("removed_items", removed);
        db.update("legacy_cleanup", completed, "migration_id=?", new String[]{LEGACY_MIGRATION});
    }

    private int removeLegacyPreferences(SharedPreferences preferences) {
        int removed = 0;
        SharedPreferences.Editor editor = preferences.edit();
        for (String key : LEGACY_PREFERENCE_KEYS) {
            if (preferences.contains(key)) { editor.remove(key); removed++; }
        }
        for (String key : preferences.getAll().keySet()) {
            if (key.startsWith(LEGACY_HASH_PREFIX)) { editor.remove(key); removed++; }
        }
        if (!editor.commit()) throw new IllegalStateException("legacy preference cleanup failed");
        return removed;
    }

    private int removeLegacyTocFiles() {
        File directory = new File(context.getFilesDir(), "toc");
        File[] files = directory.listFiles((unused, name) -> name.endsWith(".json"));
        if (files == null) return 0;
        int removed = 0;
        for (File file : files) {
            if (file.delete() || !file.exists()) removed++;
            else throw new IllegalStateException("legacy TOC cleanup failed");
        }
        return removed;
    }

    private void verifyLegacyRemoved(SharedPreferences preferences) {
        for (String key : LEGACY_PREFERENCE_KEYS) {
            if (preferences.contains(key)) throw new IllegalStateException("legacy preference remains: " + key);
        }
        for (String key : preferences.getAll().keySet()) {
            if (key.startsWith(LEGACY_HASH_PREFIX)) {
                throw new IllegalStateException("legacy hash preference remains");
            }
        }
        File directory = new File(context.getFilesDir(), "toc");
        File[] files = directory.listFiles((unused, name) -> name.endsWith(".json"));
        if (files != null && files.length > 0) throw new IllegalStateException("legacy TOC files remain");
    }

    private void verifyDatabaseForCleanup(SQLiteDatabase db) throws Exception {
        try (Cursor cursor = db.rawQuery("pragma integrity_check", null)) {
            if (!cursor.moveToFirst() || !"ok".equalsIgnoreCase(cursor.getString(0))) {
                throw new IllegalStateException("local database integrity check failed");
            }
        }
        try (Cursor cursor = db.rawQuery("pragma foreign_key_check", null)) {
            if (cursor.moveToFirst()) throw new IllegalStateException("local database foreign keys are invalid");
        }
        String managedRoot = booksDirectory.getCanonicalPath() + File.separator;
        try (Cursor cursor = db.rawQuery("select relative_path from books", null)) {
            while (cursor.moveToNext()) {
                File file = new File(context.getFilesDir(), cursor.getString(0));
                if (!file.getCanonicalPath().startsWith(managedRoot) || !file.isFile()) {
                    throw new IllegalStateException("managed book file is missing or outside storage");
                }
            }
        }
    }
