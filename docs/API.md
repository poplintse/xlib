...
旧 `DELETE /v1/progress` 与 `DELETE /v1/account` 已从当前源码及契约移除，调用返回 404；不提供兼容性的全量删除回退。这是删除接口的破坏性变更，已有部署不会因源码修改自动升级。客户端传输层已改为单书接口；用户入口、删除后本次阅读会话暂停及重开恢复尚待实现，见 [当前状态](CURRENT.md)。
