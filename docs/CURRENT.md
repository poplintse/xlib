...
- iOS 原生阅读器：书架、阅读、搜索、目录、设置和进度同步；已启动同步后，在应用前台以单个 20 秒任务读取最新本地配置并上传本地阅读进度，无活动阅读页时也同步已保存的本地进度。进入后台时尽力上传一次，受 iOS 后台调度限制不承诺持续运行。详见 [`features/ios.md`](features/ios.md)。
