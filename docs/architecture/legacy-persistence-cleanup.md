...
0.11.0 当前仍是 draft，未执行发布、tag 或 push。发布前仍需运行常规发布门禁，并验证实际分发渠道拒绝 pre-SQLite 版本直接跳到 0.11.0。未经过 0.10.0 的旧安装必须先安装 0.10.0 或重新导入书籍；0.11.0 不再携带恢复旧 metadata 的解析器。
