...
- 保存当前位置时，把当前可见首行的字符索引通过当前窗口的字符/字节映射转换为绝对 offset，不能用“窗口内字节比例 = 页面高度比例”近似换算。
...
- 本地缓存不覆盖目标 offset 时，仍先显示缓存作为即时预览，再异步加载覆盖目标 offset 的正确窗口；只有 offset 与缓存范围确实不一致时才允许发生这次跳转。
...
- 分别验证缓存覆盖和不覆盖目标 offset 两条重开路径：覆盖时不得重复渲染跳动，不覆盖时允许预览缓存后只跳转一次。
