...
public struct ByteOffsetMap: Sendable {
    private let text: String
    private let encoding: TextEncoding
    private let characterAnchors: [Int]
    private let byteAnchors: [Int]

    public init(text: String, encoding: TextEncoding, stride: Int = 128) throws {
        self.text = text
    ...
    public var totalBytes: Int { byteAnchors.last ?? 0 }

    public func byteOffset(forUTF16Index index: Int) throws -> Int {
        let source = text as NSString
    ...
    public func utf16Index(forByteOffset offset: Int) throws -> Int {
    ...
    public func utf16IndexAtOrAfter(byteOffset offset: Int) throws -> Int {
        let target = max(0, min(offset, totalBytes))
