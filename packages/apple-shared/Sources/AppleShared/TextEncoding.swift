...
public enum TextEncoding: String, Codable, CaseIterable, Sendable {
    ...
    public static func detect(at url: URL) throws -> TextEncoding {
    ...
    public func decodeCompletePrefix(_ data: Data) throws -> (text: String, bytesConsumed: Int) {
    ...
    public func encodedByteCount(of text: String) -> Int? {
        (text as NSString).data(using: foundationEncoding.rawValue, allowLossyConversion: false)?.count
    }
}

public enum ReaderCoreError: Error, Equatable, LocalizedError {
