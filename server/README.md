...
可独立部署的 XLib 阅读进度同步 API。实现以 [`docs/sync-server-design.md`](../docs/sync-server-design.md) 的第一阶段语义为准；服务只保存规范化邮箱、同步 Token、设备元数据和 TXT 的哈希/文件大小/阅读 offset，不接收原文件、书名、正文或本地路径。
...
在 `/opt/apps/xlib-sync/repo/server`：
...
```bash
/opt/apps/xlib-sync/repo/server/scripts/backup.sh
```
